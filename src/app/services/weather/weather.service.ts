...
export class WeatherService {
  ...
  getCityWeatherByName(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<string> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=${metric}&APPID=2d586a668cbbc89219e927bd7621ea02`)
      .subscribe((data) => {
  ...
  getWeatherState(city: string): Subject<string> {
    ...
    this.http.get(
      `https://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=2d586a668cbbc89219e927bd7621ea02`)
      .subscribe((data) => {
  ...
  getMaxTemp(city: string, metric: 'metric' | 'imperial' = 'metric'): Subject<number> {
    ...
    let max: number;
    this.http.get(
      `https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=${metric}&APPID=2d586a668cbbc89219e927bd7621ea02`)
      .subscribe((weather: any) => {
